...
class RnnWithLstm(object):
    ...
    def __init__(self, sentence_length, n_class, vocab_size,
        ...
        self.input_x = tf.placeholder(tf.float32, [None, sentence_length], name="input_x")
        ...
        with tf.name_scope("RNN"):
            ...
            output = tf.reshape(tf.concat(outputs, 1), [-1, n_unit])

        # Dropout in hidden layer
        with tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.embedded_chars, keep_prob=self.dropout_keep_prob)
